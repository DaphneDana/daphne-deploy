...
const AchievementMetricCard: React.FC<AchievementMetricCardProps> = ({ metric }) => {
  const content = (
    <div className="relative h-full group">
      ...
      {/* Content */}
      <div className="relative p-6 sm:p-8 h-full flex flex-col justify-center items-center text-center">
        {/* Icon */}
        <div className="relative mb-4 sm:mb-6">
          ...
          <div className="relative p-3 sm:p-4 bg-gradient-to-br from-blue-500/20 to-blue-600/30 rounded-full border border-blue-500/30 backdrop-blur-sm group-hover:scale-110 transition-transform duration-300">
            <TrendingUp size={28} className="text-blue-400 group-hover:text-blue-300 transition-colors duration-300" />
          </div>
        </div>
        
        {/* Metric Value */}
        <motion.div 
          className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-white via-blue-100 to-blue-200 bg-clip-text text-transparent mb-2 sm:mb-3"
        ...
        {/* Metric Label */}
        <div className="text-base sm:text-lg font-medium text-gray-300 group-hover:text-gray-200 transition-colors duration-300 mb-3">
          {metric.label}
        </div>
        
        {/* Description */}
        {metric.description && (
          <p className="text-xs sm:text-sm text-gray-400 group-hover:text-gray-300 transition-colors duration-300 leading-relaxed max-w-xs">
            {metric.description}
          </p>
        )}
  ...
  const cardClasses = "h-full transition-all duration-500 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20 cursor-pointer";
